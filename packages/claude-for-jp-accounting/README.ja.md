...
日本の税理士事務所向け AI 自動仕訳プラグイン。freee・弥生・MF・TKC 対応。
...
```bash
claude mcp add bantou -- npx -y @kansei-link/bantou@latest
```

または Claude Code プラグインとして：

```bash
claude plugin install bantou
```
...
記憶データは `~/.bantou/memory.json` にローカル保存されます（クラウド送信なし）。

## コマンド一覧

| コマンド | 説明 |
|---|---|
| `/bantou:setup` | 事務所の初期設定 |
| `/bantou:classify` | 単一取引の分類 |
| `/bantou:import` | CSV ファイルの取込・分類 |
| `/bantou:report` | 月次レポートの生成 |
| `/bantou:correct` | 分類修正の登録 |
| `/bantou:nightly` | 夜間バッチ処理の実行 |
